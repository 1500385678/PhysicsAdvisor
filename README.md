...
| 路由 | 用途 | 参数 |
|------|------|------|
| `GET /health` | 健康检查 + 数据规模 | — |
| `GET /graph` | 知识图谱节点 + 父子边 | `?limit=N` |
...
```json
{
  "meta":   {
    ...
    "total_cases": 24,
    "total_formulas": 87,
    "formula_branch_breakdown": {"力学": 14, "电磁学": 15, "热学": 18, "光学": 20, "近代物理": 20},
...
当前覆盖:**87 条**(力学 14 + 电磁学 15 + 热学 18 + 光学 20 + 近代物理 20,from `data/formulas/01_力学公式.md` + `02_电磁学公式.md` + `03_热学公式.md` + `04_光学公式.md` + `05_近代物理公式.md`),5/5 公式分支全部就位;距 100 目标差 13 条,后续补分支内细化或新分支(天体物理/凝聚态)。
